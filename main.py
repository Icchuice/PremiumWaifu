...
async def helpcmd(update: Update, context: CallbackContext):
    txt = "**COMMAND LIST**\n**User:**\n/clutch <name> - Claim spawned waifu\n/hunt - Random waifu\n/search <name or id> - Search waifu\n/shop - Buy waifus\n/harem - Your collection\n/bcards - Battle cards\n/balance - Check coins\n/redeem <code> - Redeem code\n/battle - Start battle, anyone can join\n/qr - Donation QR\n**Admin:**\n/upload <name> <anime> [rarity] [price] [power] - Add waifu\n/delete <id> - Delete waifu\n/give <uid> <wid> - Give waifu\n/givecoin <uid> <amt> - Give coins\n/created <amt> <code> <uses> - Create code\n/stime <msgs> - Set spawn\n/broadcast <msg> - Broadcast\n/setqr <link> - Set QR\n/setpic - Set start pic"
...
async def clutch(update: Update, context: CallbackContext):
    ...
    if guess in name or guess in anime:
...
async def qr(update: Update, context: CallbackContext):
    ...
    link = get_setting("qr")
    if photo:
        await context.bot.send_photo(update.effective_chat.id, photo, caption="**Support QR**\nScan karke support karo ❤️", parse_mode='Markdown')
    elif link:
        await update.message.reply_text(f"**Support QR**\n{link}", parse_mode='Markdown')
    else:
        await update.message.reply_text("QR abhi set nahi hai")
...
async def setqr(update: Update, context: CallbackContext):
    if update.effective_user.id!= ADMIN_ID: return
    set_setting("qr",' '.join(context.args)); await update.message.reply_text("QR set")
...
def main():
    ...
    app.add_handler(CommandHandler("broadcast", broadcast)); app.add_handler(CommandHandler("setqr", setqr))
    ...
    print("CLUTCH WAIFU BOT V23 RUNNING"); app.run_polling()
